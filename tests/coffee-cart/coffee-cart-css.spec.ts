...
test(
  "MH-2 Should add discounted Mocha after adding 3 items when accepted",
  { tag: "@regression" },
  async ({ page }) => {
    await page.locator('[data-test="Espresso"]').click();
...
test(
  "MH-3 Should skip discounted Mocha after adding 3 items",
  { tag: "@regression" },
  async ({ page }) => {
    await page.locator('[data-test="Espresso"]').click();
    await page.locator('[data-test="Espresso_Macchiato"]').click();
...
test(
  "MH-4 Should update item quantity in cart",
  { tag: "@regression" },
  async ({ page }) => {
    await page.locator('[data-test="Espresso"]').click();
    await page.locator('[aria-label="Cart page"]').click();
...
test(
  "MH-5 Should remove item from cart",
  { tag: "@regression" },
  async ({ page }) => {
    await page.locator('[data-test="Espresso"]').click();
    await page.locator('[aria-label="Cart page"]').click();
